...
class LinearMPNN(MessagePassing):
    def __init__(self, in_channels, out_channels):
        super(LinearMPNN, self).__init__(aggr='add')  # 使用加和聚合消息
        self.lin = nn.Linear(in_channels, out_channels) # need to change
        self.proj = nn.Linear(9, 1) # projection on edge features
# match shape
    def forward(self, x, edge_index, edge_attr):
        # 添加自环边，并扩展 edge_attr
        # edge_index, _ = add_self_loops(edge_index, num_nodes=x.size(0))
        #if edge_attr is not None:
        #    edge_attr = torch.cat([edge_attr, torch.zeros((x.size(0), edge_attr.size(1)))], dim=0)

        #edge_attr = torch.rand((22, 1)) 
        #print("Edge Index Shape:", edge_index.shape)
        #print("Edge Attr Shape:\n", edge_attr.shape)
        #print("Shape of x before propagate:", x.shape)
        # 进行消息传递
    ...
    def update(self, aggr_out):
        # 通过线性层
        updated_out = self.lin(aggr_out) #  
        #print("Shape of x after propagate:", updated_out.shape)
...
class GraphConv(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr): 
        # print('GraphConv_edge index location')
        # print(edge_index)
        #bs, c_in, ts, n_vertex = x.shape
        x = torch.permute(x, (0, 2, 3, 1))
...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        if self.graph_conv_type == 'cheb_graph_conv':
            ...
            x_gc = self.graph_conv(x_gc_in, edge_index, edge_attr)
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        self.mpnn = LinearMPNN(in_channels=64, out_channels=16)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        #print("==== Before GraphConv layer shape:", x.shape) # x shape [batch size, 64, 6, # of nodes]

        x = torch.transpose(x, 1, 3) 
        x = torch.transpose(x, 1, 2) # transpose to [B, X, # nodes, hidden dim]
        x = self.mpnn(x, edge_index, edge_attr) 
        x = torch.transpose(x, 2, 1)
        x = torch.transpose(x, 3, 1)
        
        #x = self.graph_conv(x, edge_index, edge_attr)  # 将 edge_index 传递给 GraphConvLayer
        
        #print(" === After GraphConv layer shape:", x.shape) # [batch size, 16, 6, # of nodes]
