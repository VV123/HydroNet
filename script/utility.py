...
def evaluate_model(model, loss, data_iter, edge_index, edge_attr):
    model.eval()
    l_sum, n = 0.0, 0
    with torch.no_grad():
...
def evaluate_metric(model, data_iter, scaler, edge_index, edge_attr):
    model.eval()
    with torch.no_grad():
        mae, sum_y, mape, mse = [], [], [], []
        for i, (x, y) in enumerate(data_iter):
            ...
            y = y.cpu().numpy()
            # print(f"y (after cpu().numpy()): {y.shape}")
            
            # Reshape y to 2D
            y = y.reshape(-1, y.shape[-1])
            # print(f"y (after reshape to 2D): {y.shape}")
            
            y = scaler.inverse_transform(y)
            # print(f"y (after inverse_transform): {y.shape}")
            
            # Reshape back to 1D
            y = y.reshape(-1)
            # print(f"y (after reshape to 1D): {y.shape}")

            y_pred = model(x, edge_index, edge_attr).cpu().numpy()
            # print(f"y_pred (after model): {y_pred.shape}")

            # Reshape y_pred to 2D
            y_pred = y_pred.reshape(-1, y_pred.shape[-1])
            # print(f"y_pred (after reshape to 2D): {y_pred.shape}")
            
            y_pred = scaler.inverse_transform(y_pred)
            # print(f"y_pred (after inverse_transform): {y_pred.shape}")
            
            # Reshape back to 1D
            y_pred = y_pred.reshape(-1)
            # print(f"y_pred (after reshape to 1D): {y_pred.shape}")
